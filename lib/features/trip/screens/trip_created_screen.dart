...
class _TripCreatedScreenState extends State<TripCreatedScreen> {
  ...
  Future<void> _replanForWeather() async {
    ...
    // 서버가 이 경로에서도 새 추천을 돌리므로 생성·재탐색과 같은 외부 AI 전송
    // 동의를 먼저 받는다. 동의가 없으면 요청 자체를 보내지 않는다.
    ...
    try {
      final response = await TripApiService.instance.replanTrip(
        scheduleId,
        canSend: () => mounted && permission.isCurrentSession,
      );
      if (!mounted) return;
      setState(() {
        _weatherAlert = null;
