...
class _TripCreatedScreenState extends State<TripCreatedScreen> {
  ...
  Future<void> _onMapReady(NaverMapController controller) async {
    ...
    // 경로 폴리라인 추가 — 구간(leg)마다 도로 추종 path 가 있으면 그 좌표를,
    // 없으면 두 stop 을 잇는 직선을 이어 붙여 하나의 내비 스타일 경로로 그린다.
    ...
    final boundsPoints =
        routeCoords.isNotEmpty ? routeCoords : _stops.map((s) => s.latLng).toList();
